...
public class UpdateProductForm extends Form{
    ...
    public UpdateProductForm(Form previous , int id, Product p)
    {
 ...
 quantity.getAllStyles().setFgColor(0xFF2261);
        ...
        Button btnInsert = new Button("Update Product");

        btnInsert.addActionListener((ActionListener) (ActionEvent evt) -> {
            
            if ((name.getText().length() == 0) || (description.getText().length() == 0) || (quantity.getText().length() == 0) || (price.getText().length() == 0)) {
                ...
                try {
                    ...
                    if (ServiceProduct.getInstance().updateProduct(id, pr)) {
